...
The script on `main` is only the bootstrap. The actual Zero-Mem 0.2.1 plugin payload is pinned to an immutable source commit and is never downloaded from mutable `main`. The exact payload commit is recorded in the bootstrap scripts and release metadata; CI asserts that they agree.
